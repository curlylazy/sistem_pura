...
function getTotal($kodeitem, $tahun, $bulan)
{
    ...
    $Sql = "SELECT tbl_item.kodeitem, SUM(tbl_itemmasukdt.jumlah) as total FROM tbl_itemmasukdt
            INNER JOIN tbl_item ON (tbl_item.kodeitem = tbl_itemmasukdt.kodeitem)
            INNER JOIN tbl_itemmasukhd ON (tbl_itemmasukhd.kodeitemmasuk = tbl_itemmasukdt.kodeitemmasuk)
            WHERE 
                DATE_FORMAT(tbl_itemmasukhd.dateitemmasuk, '%Y') = :tahun AND 
                DATE_FORMAT(tbl_itemmasukhd.dateitemmasuk, '%m') = :bulan AND
...
$(document).ready(function() {
    ...
    $("#preview").click(function() {
        $("#form1").attr("action", "<?= "index.php?page=laporan&act=grafikitemmasuk"; ?>");
        $("#form1").submit();
    });
